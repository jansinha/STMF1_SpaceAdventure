...
void audioplayerInit( unsigned int sampleRate )
{
  ...
  // SETUP GPIO pin to send analog audio output signal
  GPIO_StructInit(&GPIO_InitStructure);
  GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_4;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;
  GPIO_Init(GPIOA, &GPIO_InitStructure);

  // Establish sampling rate by calculating the prescaler
  Scaler = (uint16_t) (24000000/sampleRate);

  // SETUP Timer
  TIM_TimeBaseStructInit(&TIM_TimeBaseStructure);
  TIM_TimeBaseStructure.TIM_Period = Scaler;
